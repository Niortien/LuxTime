...
import { createWatch, deleteWatch, updateWatch } from "@/services/watch/watch.action";
import { toast } from "sonner";
import { WatchSchema, WatchUpdateSchema } from "@/services/watch/watch.schema";



export default function WatchDashboard({ watches }: { watches: Watch[] }) {
  const [selectedWatch, setSelectedWatch] = useState<Watch | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [activeTab, setActiveTab] = useState("All Watches");
const router = useRouter();
  const addWatch = async (watch: WatchSchema) => {
    const result= await createWatch(watch);
    if (result.success) {     
      toast.success("Montre ajoutée avec succès");
      router.refresh()
    }
    else {
  ...
  const handleUpdateWatch = async (updatedWatch: WatchUpdateSchema) => {
    
   
    if(!selectedWatch) return;
    const result = await updateWatch(selectedWatch?.id,updatedWatch)
    if (result.success) {
      toast.success("Montre mise à jour avec succès");
      router.refresh()
  ...
  const handleDeleteWatch = async (id: number) => {
    ...
    if (result.success) {
      toast.success("Montre supprimée avec succès");
      router.refresh()
    } else {
      toast.error(result.error);
    }
  ...
  const filteredWatches = watches.filter(
    (watch) => watch.name.toLowerCase().includes(searchTerm.toLowerCase())
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 overflow-auto bg-white rounded-lg m-4 p-6">
          ...
          <div className="bg-blue-100 rounded-lg p-4">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-500">
                Total: {watches.filter((w) => w.price > 5000).length}
              </span>
            </div>
          </div>
          ...
          <div className="bg-red-100 rounded-lg p-4">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-500">
                Total:{" "}
                {
                  watches.filter((w) => w.price <= 5000 && w.price > 1000)
                    .length
                }
              </span>
            </div>
          ...
          <div className="bg-amber-100 rounded-lg p-4">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-amber-700">
                {getProgressPercentage(2)}%
              </span>
              <span className="text-gray-500">
                Total: {watches.filter((w) => w.price <= 1000).length}
              </span>
            </div>
          </div>
        </div>
        ...
        {isFormOpen ? (
          <WatchForm
            watch={selectedWatch}
            onSave={selectedWatch ?  handleUpdateWatch : addWatch}
            onCancel={() => {
              setIsFormOpen(false);
              setSelectedWatch(null);
            }}
          ...
          <WatchList
            ...
            onDelete={async (id: number) => await handleDeleteWatch(id)}
          />
        )}
      </div>
    </div>
